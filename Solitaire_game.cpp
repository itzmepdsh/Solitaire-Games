//Submitted By: PRADEEP SHARMA (2K20/MC/098) and SAKSHI VERMA (2K20/MC/118)
...
{   char ch, choice;
	...
	while(1){
		...
		if(islem.isboardempty && islem.istopempty){//Stop while loop when board and top lists are empty
            ...
            printf ("\n");
            printf ("\t\t\t NAME:             Roll No: \n\n");
            printf ("\t\t\t PRADEEP SHARMA   2K20/MC/098 \n");
            printf ("\n\n");
            printf ("\t\t\t\t\t\t SUBMITTED TO: Ms. NEHA PUNETHA MAM\n");
...
void List::create_toplist(){
	...
	char tempcolor, tempsuit, tempvalue[4], tempisup[4], garbage[10];
	//temporary values of color,suit,isup. value will be changed to integer 
	//and isup(Up or Down) will be changed to boolean variables
	top_head = NULL;
